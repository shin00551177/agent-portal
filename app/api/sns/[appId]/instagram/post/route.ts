...
export async function POST(
  ...
  const body = await req.json().catch(() => ({})) as {
    hypothesisId?: string;
    imageUrl?: string;   // 省略時は自動生成済みアセットを使用
    caption?: string;    // 同上
  };
  ...
  // 自動生成済みアセットをデフォルトとし、bodyの明示指定があれば優先
  const stored = (hypothesis.metrics as { generatedImageUrl?: string; generatedCaption?: string } | null) ?? {};
  const imageUrl = body.imageUrl ?? stored.generatedImageUrl;
  const caption = body.caption ?? stored.generatedCaption;
  if (!imageUrl || !caption) {
    return NextResponse.json({ error: "no assets available — generate assets first or pass imageUrl/caption" }, { status: 400 });
  }

  try {
    const { mediaId } = await publishImagePost(imageUrl, caption);

    await db.snsHypothesis.update({
      where: { id: hypothesis.id },
      data: { status: "posted", postedAt: new Date(), metrics: { ...stored, igMediaId: mediaId } },
    });
    ...
    await writeAuditLog({
      ...
      afterValue: { mediaId, caption: caption.slice(0, 100) },
      req,
    });
