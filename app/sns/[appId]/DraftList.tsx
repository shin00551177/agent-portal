...
type Draft = {
  ...
  imagePrompt: string | null; imageUrl: string | null; imageStatus: string | null;
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        {draft.imageStatus === "done" && draft.imageUrl && (
          <div className="rounded-xl overflow-hidden border border-[#f0f0f0]">
            ...
            <img src={draft.imageUrl} alt="生成画像" className="w-full max-h-56 object-cover" />
          </div>
        )}
        {draft.imageStatus === "error" && draft.imagePrompt && (
          <p className="text-[12px] text-[#6e6e73] bg-[#f5f5f7] px-3 py-2 rounded-xl">
            画像プロンプト（生成失敗）: {draft.imagePrompt}
          </p>
        )}
